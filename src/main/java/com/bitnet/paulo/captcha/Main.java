        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            if (!this.getMenu().getViewMap().isEmpty()) {
                if (this.getMenu().getViewMap().containsKey(p)) {
                    for (Map.Entry<Player, InventoryView> viewEntry : this.getMenu().getViewMap().entrySet()) {
                        if (viewEntry.getValue().getTitle().equalsIgnoreCase(ChatColor.translateAlternateColorCodes('&', "&cCaptcha"))) {
                            InventoryView view = viewEntry.getValue();
                            view.close();
                        } else {
                            break;
                        }
